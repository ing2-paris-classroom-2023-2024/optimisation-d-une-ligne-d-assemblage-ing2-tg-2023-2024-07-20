...
int exclus(int sommet1, int sommet2, int** exclusions, int nb_exclusions) {
...
int prededcesseur(Sommet* sommet, int id, int nb_stations, Station* stations)
{
...
void regroupement_sommets(Sommet* sommets, int nb_sommets, int T0, int** exclusions, int nb_exclusions) {
    ...
    while (nb_sommets_positionnes<nb_sommets) // tant que tous les somments ne sont pas positionnés dans une station, on continue
    {
        ...
        {
            if (!prededcesseur(sommets, i, j+1, stations))
            {
                if (temps <= stations[j].temps_restant && !exclus(sommet, stations[j].sommets[0], exclusions, nb_exclusions)&& sommet>0)
                {
                    positionne_ce_tour++;
    ...
    printf("Affichage lignes d'assemblages avec precedences et cycles\n");
...
int precedences_et_cycle(char fichier_precedences[50], char fichier_cycle[50],char fichier_temps[50],char fichier_exclusions[50]) {
    ...
    regroupement_sommets(sommets, nb_sommets, T0, exclusions, nb_exclusions);
